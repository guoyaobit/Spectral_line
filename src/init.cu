...
PacketBatch *allocatePacketBatch(size_t numpkts)
{
    ...
    {
        batch->buffer = (Packet *)malloc(sizeof(Packet) * numpkts);
        if (!batch->buffer)
        {
            std::cerr << "malloc failed for PacketBatch buffer" << std::endl;
            delete batch;
            return nullptr;
        }
    }
    else // spectrum line mode
    {
...
int init()
{
    ...
    {
        cfg.stream_queues.emplace_back(cfg.QUEUE_CAPACITY);
    }
    cfg.stream_pools.resize(cfg.max_streams);
    if (cfg.total_nfft % 4096 != 0)
        cfg.logger_->error("total_nfft must be multiplied by 4096!");
    int batchsize = cfg.batchsize();
    cfg.logger_->debug(
        "Allocating pinned memory {} MiB", cfg.max_streams * cfg.QUEUE_CAPACITY * sizeof(Packet) * batchsize / 1e6);
    for (size_t s = 0; s < cfg.max_streams; ++s)
    {
        cfg.stream_pools[s].resize(cfg.QUEUE_CAPACITY);
        
        for (size_t b = 0; b < cfg.QUEUE_CAPACITY; ++b)
        {
            cfg.stream_pools[s][b] = allocatePacketBatch(batchsize);
        }
    }
    // init subband thread
    for (size_t i = 0; i < cfg.subbands.size(); i++)
    {
        if(cfg.subbands[i]->enable == false)
        {
            cfg.logger_->info("subband {} is disabled, skip it.", i);
            continue;
        }
        cfg.logger_->info("subband {}: starting thread on GPU {}", i, cfg.subbands[i]->gpu_id);
